...
parser.add_argument('--cpu-cores', help='number of CPU cores to use', dest='cpu_cores', type=int,
                    default=max(psutil.cpu_count() / 2, 1))
parser.add_argument('--gpu-threads', help='number of threads to be use for the GPU', dest='gpu_threads', type=int,
                    default=8)
parser.add_argument('--gpu-vendor', help='choice your GPU vendor', dest='gpu_vendor',
                    choices=['apple', 'amd', 'intel', 'nvidia'])
...
def start(preview_callback=None):
    ...
    if not args.output_file:
        ...
        args.output_file = rreplace(target_path, "/", "/swapped-",
                                    1) if "/" in target_path else "swapped-" + target_path
    target_path = args.target_path
...
def run(source_img=None, target_path=None, output_file=None, keep_fps=False, keep_frames=False, all_faces=False,
        max_memory=None, cpu_cores=None, gpu_threads=None, gpu_vendor=None):
    global args, POOL

    args = argparse.Namespace()
    args.source_img = source_img
    args.target_path = target_path
    args.output_file = output_file
    args.keep_fps = keep_fps
    args.keep_frames = keep_frames
    args.all_faces = all_faces
    args.max_memory = max_memory
    args.cpu_cores = cpu_cores if cpu_cores else max(psutil.cpu_count() // 2, 1)
    args.gpu_threads = gpu_threads if gpu_threads else 8
    args.gpu_vendor = gpu_vendor

    roop.globals.all_faces = all_faces
